...
void UIManager::showNodesTable()
{
    ...
    for (uint8_t i = 0; i < ROWS_PER_PAGE; i++)
    {
        ...
        LoRaPacket pkt = nodeData[idx];
        unsigned long elapsed = millis() - lastUpdate[idx];
        int remain = (TIMEOUT_MS - elapsed) / 1000;
        bool online = remain > 0;

        int y = top + 1 + rowHeight * (i + 1) + 2;
        display.setCursor(2, y);
        display.print(idx + 1);
        display.setCursor(22, y);
        display.print(pkt.sourceId);
        ...
        display.print(online ? F("ON") : F("OFF"));
    }
...
void UIManager::updateNode(LoRaPacket pkt)
{
    for (uint8_t i = 0; i < nodeCount; i++)
    {
        if (nodeData[i].sourceId == pkt.sourceId)
        {
            nodeData[i] = pkt;
            lastUpdate[i] = millis();
            return;
